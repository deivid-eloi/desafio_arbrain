...
export default function ParametrosPage() {
  ...
  useEffect(() => {
    cervejasService.listarTodas().then(setCervejas).catch(() => setErro('Erro ao carregar cervejas.'));
  }, []);

  // Ao selecionar uma cerveja, busca os parâmetros existentes (se houver).
  const selecionarCerveja = async (id: number) => {
    ...
    try {
      ...
      // 404 — cerveja ainda não tem parâmetros, formulário começa vazio.
      setForm(formVazio);
    }
  };
  ...
  // Cada campo renderiza um par mínimo/máximo.
  const camposParam = [
  ...
  return (
    <>
      <h1 style={{ marginBottom: '1rem', color: 'var(--color-primary)' }}>Parâmetros Fermentativos</h1>
      ...
      {/* Formulário de parâmetros — visível após selecionar cerveja */}
      {cervejaId && (
